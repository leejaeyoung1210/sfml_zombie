...
void SceneGame::Init()
{
	...
	texIds.push_back("graphics/bullet2.png");
...
{
	FRAMEWORK.GetWindow().setMouseCursorVisible(false);// 마우스 커서가리기
	...
	uiView.setCenter(windowSize * 0.f);
...
void SceneGame::Exit()
{
	FRAMEWORK.GetWindow().setMouseCursorVisible(true);// 마우스 보이기 
...
void SceneGame::SpawnZombies(int count)
{
	for (int i = 0; i < count; ++i)
	{
		...
		zombie->Reset();
		zombie->SetPosition(Utils::RandomOnUnitCircle() * 500.f);
