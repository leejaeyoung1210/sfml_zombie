...
const float Utils::PI = acosf(-1.f); //파이값을 상수로 3.14~
...
float Utils::RadianToDegree(float radian)//단위변환
{
    return radian * (180.f / PI);
}

float Utils::DegreeToRadian(float degree)//단위변환
{
    return degree * (PI / 180.f);
}

float Utils::AngleRadian(const sf::Vector2f& vec)//라디안으로 각도 리턴
{
    return std::atan2(vec.y, vec.x);
}

float Utils::Angle(const sf::Vector2f& vec)//디그리로 각도리턴
{
